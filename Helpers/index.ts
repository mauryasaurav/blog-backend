...
export const withErrorHandler = async (req: any, res: any, apiFunction: Function) => {
    console.log("Starting ...", req.originalUrl);
    try {
        await apiFunction(req, res);
    } catch (error) {
        console.log("Error- " + req.baseUrl + error);
        return sendErrorResponse(res, { "error": { "message": "Something went wrong" }, "statusCode": STATUS_CODES.INTERNAL_SERVER_ERROR });
    }
};
...
export const sendErrorResponse = (res: any, errorResponse: errorResponse) => res.status(errorResponse.statusCode).send({ "success": false, "error": errorResponse.error })
